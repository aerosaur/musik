...
public class SearchPage: DestroyablePage {
    ...
    private var needsFullRerender: Bool = false
    ...
    public func onResize(newPageState: PageState) async {
        ...
        pageNamePlane.updateByPageState(.init(absX: 2, absY: 0, width: 18, height: 1))

        itemIndicesPlane.erase()
        itemIndicesPlane.updateByPageState(.init(absX: 1, absY: 1, width: 3, height: state.height - 2))
    ...
    private func updateSelectionIndicator() -> Bool {
        ...
        for i in 0..<itemCount {
            ...
            if realIndex == selectedIndex {
                itemIndicesPlane.setColorPair(colorConfig.pageName)
                itemIndicesPlane.putString(">\(realIndex)", at: (x: 0, y: 2 + Int32(i) * 5))
            } else {
                itemIndicesPlane.setColorPair(colorConfig.itemIndices)
                itemIndicesPlane.putString(" \(realIndex)", at: (x: 0, y: 2 + Int32(i) * 5))
            }
    ...
    private func updateColumnsSelectionIndicator(
        ...
        for (i, col) in columns.enumerated() {
            ...
            for j in 0..<col.cache.count {
                ...
                let isSelected = (i == colType && realIndex == selectedIndex)
                col.indicesPlane?.setColorPair(isSelected ? activeColor : dimColor)
                let marker = isSelected ? ">" : " "
                col.indicesPlane?.putString("\(marker)\(realIndex)", at: (x: 0, y: 2 + Int32(j) * 5))
            }
            col.indicesPlane?.setColorPair(dimColor)
        }
    }
    ...
    public func render() async {
        ...
        if case .multiSearchResult(_) = result {
            ...
            if scrollChanged, case .searchResult(let sr) = result {
                ...
                // Force selection indicator redraw on next frame
                // (update only draws plain numbers, not the > marker)
                lastSelectedIndex = -1
                return
            }
        }
        ...
        if needsFullRerender {
            ...
            lastSelectedIndex = -1
            lastSelectedColumn = -1
            // Tear down the page queue so the switch below re-renders
            while SearchPage.searchPageQueue.size() > 0 {
                await SearchPage.searchPageQueue?.page?.destroy()
                SearchPage.searchPageQueue = SearchPage.searchPageQueue?.previous
            }
